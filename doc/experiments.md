...
>[!IMPORTANT]
>In this report two different experiments are featured. *Alternative* traversability is a small variation of the standard criterion. *Adaptive* traversability is a broader redefinition based on local geometry and is the main focus of the project.

## Alternative Traversability Criteria

In the vanilla algorithm a gate is traversable if the radius of its minimum Delaunay circle is $\geq \alpha$. This definition can be motivated with the idea of using a circular spoon of radius $\alpha$ to carve out the inside cell without touching any of the vertices.

The minimum Delaunay circle corresponds to one of the following 3 cases:

1. It is the smallest circle through the gate vertices (left image)
...
>[!NOTE]
>Shown in the images is a *gate* (orange) with an adjacent *inside* cell (blue) and an adjacent *outside* cell (white). Displayed in gray are the circumcircles of these cells. The red circle indicates the minimum Delaunay circle. (The red dot has no meaning; it was just used as a handle in GeoGebra.)
...
- The modified version takes fewer iterations to terminate.
- There is no significant change in runtime.
- There is no easily recognizable visual change in the produced output. 

It is to be expected that the modified version takes fewer iterations as we strengthen the condition for traversability, i.e., consider fewer gates for processing. The cases where the modification influences traversability seem rare enough for it not to make a noticeable difference. This topic could be explored further (e.g., testing with different parameters, using a wider range of inputs, or implementing it in the 3D algorithm). From this first look, it seems to have a tiny impact on how the algorithm behaves.
...
For both methods this parameter allows us to control how large a hole needs to be for it to be considered traversable. A large target length results in a less detailed evaluation of a gate where small features are less likely to affect traversability. A small target length allows small input features to have a bigger influence on the traversability of the gate.
...
**Note**: In this section the offset parameter is denoted as $\epsilon$.
...
Below is a runtime comparison for the four different configurations. In these plots the runtime for all 8 examples is shown. For each example it is shown how time was distributed across each task of the algorithm, where:
...
- The adaptive methods insert fewer vertices.
...
- Intersection-based traversability more consistently produces desirable results and usually terminates faster than deviation-based traversability. In particular, the deviation-based method fails to produce a satisfactory result when increasing the offset.
...
- To make the results comparable, all methods used the same value for $\alpha$ (or target length). However, better results might be achieved by choosing the remaining traversability parameters of the adaptive methods differently. 
- The implementation of the procedure for updating the queue can be made more efficient. Currently, the queue is cleared and rebuilt by checking every facet in the triangulation. The adaptive methods would especially benefit from a more efficient implementation, as checking gates for traversability currently takes up most of their runtime.
- The methods were compared on a small dataset of rather simple example inputs described by *dense* point clouds. Behavior on messy, real-world data might be different.
...
Intersection-based traversability appears to be a viable approach for adaptive traversability. From these experiments it cannot be concluded yet whether it can produce satisfactory results for 3D real-world inputs within reasonable runtime. This could be explored as future work, as it is fairly simple to extend this modification to the 3D algorithm.